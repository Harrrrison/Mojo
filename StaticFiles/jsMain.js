...
fetch('https://api.spotify.com/v1/me/top/tracks?time_range=short_term&limit=10', {
    ...
    .then(data => {
        try {
            const songIDs = data.items.slice(0, 10).map(song => song.id); // Extract song names. Used this as there are multiple names held. May need to be changed if we vary the number of songs we display
            // Convert songNames array to a string for display, e.g., as a list
            const IDList = songIDs.map(name => `<li>${name}</li>`).join('');
            for (let i = 0; i < songIDs.length; i++) {
                fetch(`https://api.spotify.com/v1/audio-features/${IDList[i]}`, {
                    headers: { 'Authorization': 'Bearer ' + accessToken }
                    })
                    .then(response => response.json())
                    .then(data => {
                        try {
                            const test = data; // Extract song names. Used this as there are multiple names held. May need to be changed if we vary the number of songs we display
                            // Convert songNames array to a string for display, e.g., as a list
